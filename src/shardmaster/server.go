...
//raft状态机响应后通知channel返回给客户端消息 - 240行
...
func (sm *ShardMaster) updateConfig(op string, arg interface{}) {
	...
	if op == "Move" {
		...
		for gid, servers := range joinArg.Servers {
			...
			//debug.PrintStack()
			//log.Printf("Join : %v, %v, %v", gid, servers, cfg)
		}
	} else if op == "Leave" {
...
func (sm *ShardMaster) rebalance(cfg *Config, request string, gid int) {
	...
	case "Join":
		...
		for i := 0; i < avg; i++ {
			...
			//log.Printf("rebalance : %v, %v", cfg, shardsCount)
...
func (sm *ShardMaster) groupByGid(cfg *Config) map[int][]int {
	...
	for k, _ := range cfg.Groups {
		//log.Printf("k: %v, %v", k, cfg.Groups)
		shardsCount[k] = []int{}
	}
	for k, v := range cfg.Shards {
		//log.Printf("k: %v, v: %v, %v", k, v, shardsCount)
	...
	go func() {
		for {
			...
			case applyMsg := <-sm.applyCh:
				...
				//log.Printf("%v, apply : %v, %v, %v, %v, %v", me, op.OpType, op.Cid, op.SeqNum, found, maxSeq)
